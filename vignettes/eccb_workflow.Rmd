...
observed_plot <- scater::plotColData(tse, 
                           y = "observed", 
                           x = "disease",
                           colour_by = "disease") + 
...
shannon_plot <- scater::plotColData(tse, 
                           y = "shannon", 
                           x = "disease",
                           colour_by = "disease") +
  geom_signif(comparisons = comb, 
...
Sometimes, we also want to create elipse to guide our visualization wheter the microbial community in two groups is different. You achieve it using the code below.

```{r PCoA elipse}
# Add ellipses with ggplot manual

# Extract PCoA coordinates
pcoa_coords <- as.data.frame(reducedDim(tse, "MDS_bray"))
colnames(pcoa_coords) <- c("PC1", "PC2")

# Add metadata
pcoa_coords$disease <- colData(tse)[["disease"]]

# Filter out NA values in the variable column
pcoa_coords <- na.omit(pcoa_coords)

# Create the plot with ellipses
p_elipse <- ggplot(pcoa_coords, aes(x = PC1, y = PC2, color = disease)) +
    geom_point(alpha=0.8, size=0.5) +
    theme_classic() + 
    stat_ellipse(aes(color = disease), level = 0.95) +  # Ellipses based on disease
    labs(x = paste("PC1 (", round(100 * rel_eig[1], 1), "%)", sep = ""),
         y = paste("PC2 (", round(100 * rel_eig[2], 1), "%)", sep = ""),
         title = "PCA Plot with Ellipses") +
...
OMA book also provide further exploration to explore the microbial community structure in the table, for example based on [community typing](https://microbiome.github.io/OMA/docs/devel/pages/clustering.html). 


Another primary analysis that usually can be done 
in microbiome study is **differential abundancemanalysis (DAA)** 
analysis for example to answer the research question 3. 
